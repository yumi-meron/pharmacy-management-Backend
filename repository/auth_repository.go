...
type AuthRepository interface {
	...
	SaveResetToken(ctx context.Context, userID uuid.UUID, token string, expiry time.Time) error
...
func (r *authRepository) Create(ctx context.Context, user domain.User) error {
	...
	if err != nil {
		if err.Error() == "pq: duplicate key value violates unique constraint \"users_phone_number_key\"" {
			r.logger.Error().Err(err).Msg("Phone number already taken")
...
// SaveResetToken stores a password reset token
func (r *authRepository) SaveResetToken(ctx context.Context, userID uuid.UUID, token string, expiry time.Time) error {
	query := `
	...
	id := uuid.New()
	now := time.Now()
	_, err := r.db.ExecContext(ctx, query, id, userID, token, expiry, now, now)
	if err != nil {
...
// DeleteResetToken removes a reset token
...
func (r *authRepository) SaveRefreshToken(ctx context.Context, userID uuid.UUID, token string, expiry time.Time) error {
	query := `
